...
//?}
        ...
        graphics.uiText(font, titleText, (width - font.width(titleText)) / 2, 20, ColorUtil.WHITE)

        val stats = Component.translatable("screen.changelog363.stats", rows.size).string
        graphics.uiText(font, stats, LIST_LEFT, 35, ColorUtil.GREY)
    ...
    private fun renderUpdateStatus(graphics: UiGraphics, stats: String) {
        if (
        ...
        if (rendered.isNotEmpty()) graphics.uiText(font, rendered, left, 35, ColorUtil.YELLOW)
    }

    private fun renderWarningBanner(graphics: UiGraphics) {
        ...
        if (rendered.isNotEmpty()) graphics.uiText(font, rendered, LIST_LEFT, 45, ColorUtil.YELLOW)
    }

    private fun renderEmptyState(graphics: UiGraphics) {
        val message = when {
        ...
        graphics.uiText(font, rendered, x, y, ColorUtil.GREY)
    }

    private fun renderRows(graphics: UiGraphics) {
        graphics.enableUiScissor(LIST_LEFT, LIST_TOP, listRight, listBottom)
        ...
        graphics.disableUiScissor()
    }

    private fun renderRow(graphics: UiGraphics, row: Row, top: Int, hovered: Boolean) {
        graphics.fill(
            LIST_LEFT,
            top + 1,
            listRight,
        ...
        graphics.uiText(font, row.versionText, TEXT_LEFT, top + 4, row.versionColor)

        var badgeX = row.badgeX
        for (badge in row.badges) badgeX = graphics.drawBadge(font, badge, badgeX, top + 3)

        if (row.date.isNotBlank()) graphics.uiText(font, row.date, row.dateX, top + 4, ColorUtil.GREY)
        if (row.title.isNotBlank()) graphics.uiText(font, row.title, TEXT_LEFT, top + 18, ColorUtil.LIGHT_GREY)
        if (row.summary.isNotBlank()) graphics.uiText(font, row.summary, TEXT_LEFT, top + 34, ColorUtil.GREY)
    }
...
//?}
    ...
    private fun renderScrollbar(graphics: UiGraphics) {
