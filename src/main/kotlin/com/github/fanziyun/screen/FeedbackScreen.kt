...
//? if <1.21.11 {
                CycleButton.builder<FeedbackEndpoint>({ Component.literal(it.displayName.ifBlank { it.baseUrl }) })
                    .withValues(configuredEndpoints)
                    .withInitialValue(configuredEndpoints[0])
//?} else {
...
//? if <1.21.11 {
            CycleButton.builder<AuthMode>({
                ...
                .withInitialValue(AuthMode.OAUTH)
//?} else {
            CycleButton.builder<AuthMode>({
                if (it == AuthMode.OAUTH) Component.translatable("screen.changelog363.feedback.auth.oauth")
                else Component.translatable("screen.changelog363.feedback.auth.pat")
...
//? if <1.21.6 {
        val content = MultiLineEditBox(
            font,
            left + SMALL_GAP,
            CONTENT_BOX_Y,
            PANEL_W - SMALL_GAP * 2,
            CONTENT_BOX_H,
            Component.literal(contentPlaceholder),
            Component.translatable("screen.changelog363.feedback.label"),
        )
//?} else {
...
//? if <1.21.9 {
        pat.setFormatter { value, _ -> FormattedCharSequence.forward("•".repeat(value.length), Style.EMPTY) }
//?} else {
...
//? if <26 {
    override fun render(graphics: UiGraphics, mouseX: Int, mouseY: Int, partialTick: Float) {
//?} else {
    override fun extractRenderState(graphics: UiGraphics, mouseX: Int, mouseY: Int, partialTick: Float) {
//?}
        drawPanel(graphics)
//? if <26 {
...
//?}
        ...
        graphics.uiText(font, currentTitle, (width - font.width(currentTitle)) / 2, 20, ColorUtil.WHITE)
        drawLabels(graphics)
        drawStatus(graphics)
        drawLoginInfo(graphics)
    }

    private fun drawPanel(graphics: UiGraphics) {
        val left = (width - PANEL_W) / 2
    ...
    private fun drawLabels(graphics: UiGraphics) {
        val left = (width - PANEL_W) / 2
        val titleLabel = Component.translatable("screen.changelog363.feedback.title_label").string
        graphics.uiText(font, titleLabel, left + SMALL_GAP, TITLE_LABEL_Y, ColorUtil.LIGHT_GREY)
        val contentLabel = Component.translatable("screen.changelog363.feedback.label").string
        graphics.uiText(font, contentLabel, left + SMALL_GAP, CONTENT_LABEL_Y, ColorUtil.LIGHT_GREY)
        val contactLabel = Component.translatable("screen.changelog363.feedback.contact").string
        graphics.uiText(font, contactLabel, left + SMALL_GAP, CONTACT_LABEL_Y, ColorUtil.LIGHT_GREY)
    }

    private fun drawStatus(graphics: UiGraphics) {
        val message = statusMessage ?: return
        val rendered = font.ellipsize(message.string, (PANEL_W - 4).coerceAtLeast(0))
        graphics.uiText(font, rendered, (width - font.width(rendered)) / 2, STATUS_Y, statusColor)
    }

    private fun drawLoginInfo(graphics: UiGraphics) {
        ...
        graphics.uiText(font, font.ellipsize(uriText, (PANEL_W - 4).coerceAtLeast(0)), (width - PANEL_W) / 2, LOGIN_INFO_Y, ColorUtil.LIGHT_GREY)
        graphics.uiText(font, font.ellipsize(codeText, (PANEL_W - 4).coerceAtLeast(0)), (width - PANEL_W) / 2, LOGIN_INFO_Y + 14, ColorUtil.GREEN)
    }
