...
//?}
        ...
        graphics.uiText(font, headlineText, (width - font.width(headlineText)) / 2, 20, entry.color)
        if (dateText.isNotEmpty()) {
            graphics.uiText(font, dateText, (width - font.width(dateText)) / 2, 35, ColorUtil.GREY)
        }
    ...
    private fun renderChanges(graphics: UiGraphics) {
        ...
        graphics.enableUiScissor(CONTENT_LEFT, CHANGES_TOP, contentRight, contentBottom)
        var y = CHANGES_TOP - smoothScroll.toInt()
        for (line in lines) {
            if (y > contentBottom) break
            if (y + LINE_HEIGHT >= CHANGES_TOP) {
                graphics.uiText(font, line.text, CONTENT_LEFT + line.indent, y, ColorUtil.LIGHT_GREY)
            }
            y += LINE_HEIGHT
        }
